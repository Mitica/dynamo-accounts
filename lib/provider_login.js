...
internal.findAccountConnection = function findAccountConnection(data, profile, accessData) {
	var info = UserProfile.convertToAccountConnection(profile);
	info.appId = data.appId;

	var id = AccountConnectionModel.formatId(info);
	...
	return data.access.getConnectionsById({
		...
		.then(function(itemsById) {
			...
			return data.access.findConnectionsByEmail({
					email: info.email
				})
				.then(function(itemsByEmail) {
					if (itemsByEmail && itemsByEmail.length > 0) {
						...
						info.accountId = connection.accountId;
						info.accessData = accessData;

						console.log('creating from email connection', info);

						return data.control.createAccountConnection(info);
					}
				});
		});
};
